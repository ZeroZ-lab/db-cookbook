...
- **第一部分：PostgreSQL 建立数据库直觉**：让读者先理解数据如何被组织、约束、查询、优化，以及单机数据库为什么会出现边界。
- **第二部分：从业务库走向分析系统**：解释 OLTP / OLAP 分工、数仓建模和 ETL / CDC，让读者理解数据平台不是工具堆叠，而是系统目标分化后的结果。
- **第三部分：大数据计算与湖仓底座**：把批处理、实时计算和 OLAP 引擎放到同一条链路中，理解历史分析、实时分析和交互分析的不同压力。
- **第四部分：AI 时代的数据基础设施**：把向量、图、湖仓和治理纳入统一数据系统，理解 AI 应用为什么依赖可信、可追溯、可检索的数据底座。
- **第五部分：项目、路线和能力闭环**：用项目实战、学习顺序、能力地图和最终目标，把全书从知识阅读落到可执行训练。
