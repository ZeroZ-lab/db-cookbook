---
title: "01.2 PostgreSQL 核心结构：数据系统是如何被组织起来的"
description: "1.2 PostgreSQL 核心结构：数据系统是如何被组织起来的  很多人第一次接触数据库时，会自然地把它理解成“一堆表”。  这个理解有一定道理，但不够准确。  如果数据库只是几张表，那么它和 Excel 的区别并不明显。真正让数据库成"
prev: { text: "01.1 为什么要从 PostgreSQL 开始理解数据系统？", link: "/chapters/01/01-1" }
next: { text: "01.3 必学概念：从主键、外键、约束到事务、索引和分区", link: "/chapters/01/01-3" }
---
