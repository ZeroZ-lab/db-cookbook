---
title: "05.5 常见分层模型详解"
description: "5.5 常见分层模型详解  上一节讲了分层的必要性。这一节展开 ODS、DWD、DWS、ADS 四层各自该怎么做。   ODS 层：原始副本，不做加工  ODS（Operational Data Store）的职责只有一个：把源数据搬过来，"
prev: { text: "05.4 数仓分层的必要性", link: "/chapters/05/05-4" }
next: { text: "05.6 分层的实施策略", link: "/chapters/05/05-6" }
---
### 5.5 常见分层模型详解
