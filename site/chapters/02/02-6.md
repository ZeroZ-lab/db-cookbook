---
title: "02.6 窗口函数：在保留明细的同时做组内分析"
description: "2.6 窗口函数：在保留明细的同时做组内分析  聚合分析会压缩明细，计算统计指标。  但有些分析需要既保留明细，又在组内做计算：  text 每个用户的订单金额排名 每个商品在各月的销量排名 每个用户的累计消费金额 每天的订单金额与7天移动"
prev: { text: "02.5 复杂分析SQL：把中间过程表达清楚", link: "/chapters/02/02-5" }
next: { text: "02.7 指标计算：从SQL到指标", link: "/chapters/02/02-7" }
---
