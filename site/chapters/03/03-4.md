---
title: "03.4 表空间与存储策略：让数据存储更高效"
description: "3.4 表空间与存储策略：让数据存储更高效  前面学习了分区表，通过物理边界优化查询和维护。  但分区表还引出一个问题：不同分区的存储成本和访问频率不同，能否优化存储？  场景： yaml 最近3个月的订单（热数据）：   - 访问频繁  "
prev: { text: "03.3 分区表：让大表具有物理边界", link: "/chapters/03/03-3" }
next: { text: "03.5 索引基础：B-tree与查询加速", link: "/chapters/03/03-5" }
---
