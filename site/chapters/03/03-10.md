---
title: "03.10 批量操作与数据链路接入：高效导入导出大量数据"
description: "3.10 批量操作与数据链路接入：高效导入导出大量数据  前面学习了多种大表优化手段： - 分区表、索引、查询优化、物化视图、并行查询  这些都是针对已经存储在数据库中的数据的优化。  但还有一个重要的问题：如何高效地将大量数据导入导出？ "
prev: { text: "03.9 并行查询：利用多核CPU加速大表查询", link: "/chapters/03/03-9" }
next: { text: "4. OLTP vs OLAP：大数据系统的第一分水岭", link: "/chapters/04/" }
---
