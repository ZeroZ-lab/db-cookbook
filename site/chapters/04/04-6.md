---
title: "04.6 OLAP的本质：面向分析查询"
description: "4.6 OLAP的本质：面向分析查询  前面学习了OLTP的本质（面向业务事务）和优化策略。  现在进入OLAP的世界。  场景： yaml 数据分析师：   - 需要分析最近30天的GMV趋势   - 需要计算用户留存率   - 需要分析"
prev: { text: "04.5 OLTP的优化策略", link: "/chapters/04/04-5" }
next: { text: "04.7 OLAP的数据模型：维度建模与宽表", link: "/chapters/04/04-7" }
---
### 4.6 OLAP的本质：面向分析查询
