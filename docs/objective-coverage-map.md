...
- 内容完成度以 [completion-audit.md](completion-audit.md) 为唯一人工判断源。
- 目标拆解与覆盖映射以本文档为唯一目标映射源。
- 项目运行状态以 `project-workbench/project-manifest.json` 为唯一运行状态源。
- 事实核查状态以 `docs/fact-check-matrix.md` 为唯一状态源。
...
| 实战任务和项目闭环 | 每章“实战任务”、`project-workbench/`、`project-workbench/project-manifest.json`、`docs/project-runbook.md`、`site/projects.md` | `verify-book-structure`、`projects:verify` | 已有任务、清单、机器可读状态表、执行总表和站点状态入口，完整可运行项目待补 |
...
| `pnpm projects:verify` | `project-manifest.json`、`docs/project-runbook.md`、`site/projects.md`、项目目录、README、交付清单、项目 1 运行入口、项目 2 ClickHouse 关键产物、项目 3 CDC / Kafka / Flink / Sink 关键产物、项目 4 湖仓关键产物、项目 5 RAG 关键产物、项目 6 GraphRAG 关键产物、项目 7 治理关键产物 | 不证明七个项目端到端可运行 |
